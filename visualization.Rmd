...
```{r vis.penguinPlots, fig.cap="Flipper length by species"}
...
```{r vis.pivot4plot, warning=F, message=F}
...
```{r vis.histogram, warning=FALSE, message=F, fig.cap="Distribution of NDVI, Knuthson Meadow"} 
...
```{r vis.histogram2, warning=FALSE, message=F, fig.cap="Distribution of Average Monthly Temperatures, Sierra Nevada"}
sierraData %>%
...
```{r vis.histogram3, warning=FALSE, fig.cap="Cumulative Distribution of Average Monthly Temperatures, Sierra Nevada"}
...
```{r vis.density, warning=F, fig.cap="Density plot of NDVI, Knuthson Meadow"}
XSptsPheno %>% 
  ggplot(aes(NDVI)) + 
  geom_density()
```
...
```{r vis.density.eucoak, warning=FALSE, fig.cap="Runoff under Eucalyptus and Oak in Bay Area sites"}
tidy_eucoak %>%
...
```{r vis.boxplot.eucoak, warning=FALSE, fig.cap="Runoff under Eucalyptus and Oak, Bay Area Sites"}
ggplot(data = tidy_eucoak) +
  geom_boxplot(aes(x = site, y = runoff_L))

```
**Get color from tree within `aes()`**
```{r vis.boxplot.eucoak.col, warning=FALSE, fig.cap="Runoff at Bay Area Sites, colored as Eucalyptus and Oak"}
ggplot(data = tidy_eucoak) +
...
```{r vis.marble.boxplot, warning=FALSE, fig.cap = "Visualizing soil CO_2_ data with a Tukey box plot"}
soilCO2 <- soilCO2_97
...
```{r vis.sierra.scatterplot, warning=FALSE, fig.cap="Scatter plot of February temperature vs elevation"}
...
```{r vis.sierra.scatterplot.col, warning=FALSE}
ggplot(data=sierraFeb) + 
...
```{r vis.eucoak.bar, warning=FALSE, fig.cap="Two variables, one discrete"}
ggplot(tidy_eucoak) +
...
```{r vis.sierra.color, warning=FALSE, fig.cap="Color set within aes()"}
ggplot(data=sierraFeb) + 
...
```{r vis.sierra.color.line, warning=FALSE, fig.cap="Using aesthetics settings for both points and lines"}
sierraFeb %>%
...
```{r vis.longprof, fig.cap="Longitudinal Profiles"}
...
```{r vis.sierra.trendline, warning=F, message=F, fig.cap="Trend line using geom_smooth with a linear model"}
sierraFeb %>%
  ggplot(aes(TEMPERATURE,ELEVATION)) +
  geom_point(color="blue") +
...
```{r vis.TRI.col, message=F, warning=F, fig.cap="EPA Toxic Release Inventory, as a big data set needing symbology clarification"}
...
```{r vis.phenogroup.col, warning=FALSE, fig.cap="NDVI symbolized by vegetation in two seasons"}
XSptsPheno %>%
  ggplot() +
...
```{r vis.eucoak.rainrunoff.col, warning=FALSE, fig.cap="Eucalyptus and Oak: rainfall and runoff"}
ggplot(data = tidy_eucoak) +
...
```{r vis.parallel.graphs, warning=FALSE, fig.cap="Faceted graph alternative"}
ggplot(data = tidy_eucoak) +
...
```{r vis.titles, warning=FALSE, fig.cap="Titles added"}
ggplot(data = tidy_eucoak) +
...
```{r vis.pairs, warning=FALSE, fig.cap="Pairs plot for Sierra Nevada stations variables"}
sierraFeb %>%
