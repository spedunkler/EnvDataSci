...
**Plot individually**
```{r intro.plot1, fig.cap="Temperature"}
plot(temp)
```
```{r intro.plot2, fig.cap="Elevation"}
plot(elev)
```
```{r intro.plot3, fig.cap="Latitude"}
plot(lat)
```

**Then plot as a scatterplot**
```{r intro.scatter, fig.cap="Temperature~Elevation"}
plot(elev,temp)
```
#### Named indices
...
```{r intro.plotSierra, fig.cap="Temperature~Elevation", warning=F}
