...
I couldn't figure out how to pull the bands out of the list, so I'm reading them in again.

```{r}
file.path(dtaPath, "crop", imgDate, "B04.tif")

red <- raster(paste0(dtaPath, "/crop",imgDate,"/B04.tif"))
nir <- raster(paste0(dtaPath, "/crop",imgDate,"/B08.tif"))
#ndvi <- (nir - red)/(nir + red)

#names(ndvi) <- "ndvi"
#mapview(ndvi)
# This also works but requires some redundancy (list id number + name)
#redL <- rst_lst$B04
#nirL <- rst_lst["B08"]$B08
#ndviL <- (nirL - redL)/(nirL + redL)
#mapview(ndviL)

#rst_newlst <- c(rst_lst, ndvi)
# rst_newlst <- append(rst_lst,ndvi)   # does the same thing, but ndvi just becomes 11
#names(rst_newlst)[11] <- "ndvi"      # this changes the name (names is both a getter and setter)
#mapview(rst_newlst[11])
#ndvi2 <- rst_newlst[11]
#b2 <- rst_newlst[1]
#mapview(ndvi2)

rst_newlst$ndvi <- (nir - red)/(nir + red)
mapview(rst_newlst$ndvi)
#names(rst_newlst$ndvi) <- "ndvi"
...
### including NDVI:
```{r prepPredictionNew}
rst_for_predictionNew <- vector(mode = "list", length = length(rst_newlst))
names(rst_for_predictionNew) <- names(rst_newlst)
```
...
### with NDVI:
works after jumping through some hoops...

```{r resampleNew}
for (b in c("B05", "B06", "B07", "B8A", "B11", "B12")){
  beginCluster(n = round(3/4 * detectCores()))  # [raster]
  try(
    rst_for_predictionNew[[b]] <- raster::resample(x = rst_newlst[[b]],
                                                   y = rst_newlst$B02)  
...
rst_for_predictionNew[b_10m] <- rst_newlst[b_10m]
rst_for_predictionNew[[11]] <- ndvi  # should be 10m

brick_for_predictionNew <- brick(rst_for_predictionNew)  # [raster]  FAILS!!!
```
