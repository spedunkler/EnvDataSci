...
```{r echo=F, fig.cap="Study of runoff & erosion under Eucalyptus & Oak canopy"}
...
```{r warning=F, fig.cap="Tukey boxplot of runoff under Eucalyptus canopy"}
...
```{r abstraction.groupSummary, warning=F, message=F}
...
```{r abstraction.summaryTRI, message=FALSE, warning=F}
...
```{r abstraction.ex5, include=FALSE}
...
```{r abstraction.ex6, include=FALSE}
...
```{r abstraction.ex7, include=FALSE}
...
```{r abstraction.ex8, include=FALSE}
penguins %>%
  arrange(body_mass_g)
```
...
```{r abstraction.str.matches, warning=F}
str_detect(fruit,"qu")
